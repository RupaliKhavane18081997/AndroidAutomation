...
import pom.check.funct.AddingNewFireStation;
import pom.createIncident.baseIncident;
import utils.ConstantsConfig;
import utils.DataUtil;


public class Firestaiontest extends BaseTest{
	baseIncident   createIncident;
	String testCaseName="AddNewFireStation";
	...
	public void TestCase01ohioautomation(Hashtable<String,String> data) throws Exception {
		...
		else
		{
			...
			AddingNewFireStation AddingNewFireStation  = new AddingNewFireStation();
		}
	}
	
	
	@Test(dataProvider = "getData", priority = 2)
	public void AddingNewFireStation(Hashtable<String,String> data)throws Exception {
		if(!DataUtil.isTestExecutable(xls, testCaseName) ||  data.get(ConstantsConfig.RUNMODE_COL).equals("No"))
		{
			test.log(Status.SKIP, "Skipping the test as Runmode is No");
			LogManager.getLogger().info("Skipping the test as Runmode is No");
		...
		{
//			myFDTab  myFDTab = new myFDTab();
//			myFDTab.myfd(data.get("FDID"));
//			myFDTab.scrollfirestation();
//			myFDTab.firestationclick();
			Firestationpage Firestationpage = new Firestationpage();
			Firestationpage.checkdetails();
			Firestationpage.firestationbackbtn();
		}
	}
}